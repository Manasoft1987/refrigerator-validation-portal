...
function translateEnglishSystemText(value: string): string {
  const replacements: Array<[RegExp, string]> = [
    ...
    [/Количество и позиции логгеров приняты по риск-ориентированной фактической схеме\.?/g, "Logger count and positions follow the risk-based actual placement scheme."],
    [/Внешний/g, "External"],
    [/Внутренний/g, "Internal"],
    [/шт\./g, "pcs"],
    [/цветовая интерполяция по средним значениям внутренних датчиков/g, "colour interpolation based on internal logger averages"],
    [/Диапазон/g, "Range"],
    [/После этапа PQ\/PV/g, "After the PQ/PV stage"],
    [/До этапа PQ\/PV/g, "Before the PQ/PV stage"],
    [/Независимо/g, "Independent"],
    [/Не вышел/g, "No excursion"],
    [/Не вош[её]л/g, "Did not enter range"],
    [/Момент включения \(T_on\)/g, "Power-on time (T_on)"],
    [/Момент стабилизации \(T_stable\)/g, "Stabilization time (T_stable)"],
    [/Длительность выхода на режим/g, "Time to reach setpoint"],
    [/Критический датчик \(последним вошедший в диапазон\)/g, "Critical logger (last to enter range)"],
    [/Вхождение датчиков в целевой диапазон/g, "Logger entry into target range"],
    [/Датчик/g, "Logger"],
    [/Т при вкл\., °C/g, "Temperature at power-on, °C"],
    [/Вхождение в диапазон/g, "Entry into range"],
    [/Длительность/g, "Duration"],
    [/Момент выхода/g, "Exit time"],
    [/Время до выхода/g, "Time to exit"],
    [/Не зафиксировано/g, "Not observed"],
    ...
    [/Время до нарушения режима \(первый датчик\)/g, "Time to first excursion"],
    [/Критический датчик \(первым вышедший из диапазона\)/g, "Critical logger (first outside range)"],
    [/Критический датчик \(первым вышедший за пределы\)/g, "Critical logger (first outside range)"],
    [/Отключение питания/g, "Power failure"],
    [/Выход датчиков за пределы диапазона \(по каждому датчику\)/g, "Logger excursions outside range (per logger)"],
    [/Тест завершён: первый датчик вышел за пределы диапазона через/g, "Test completed: the first logger left the range after"],
    [/после открытия двери/g, "after door opening"],
    [/после отключения питания/g, "after power failure"],
    [/до конца окна записи/g, "until the end of the recording window"],
    [/Тест завершён/g, "Test completed"],
    [/Тест не завершён/g, "Test not completed"],
    [/не все датчики вошли в целевой диапазон за период записи/g, "not all loggers entered the target range during the recording period"],
    [/температурный режим сохранён/g, "the temperature range was maintained"],
    [/Критический датчик:/g, "Critical logger:"],
    [/Оборудование вышло на целевой температурный режим за/g, "The equipment reached the target temperature range in"],
    [/после включения/g, "after power-on"],
    [/Тест —/g, "Test —"],
    [/Temperature Excursion Study \(Temperature Excursion Study\)/g, "Temperature Excursion Study"],
...
function verificationTerminology(text: string | null | undefined): string {
  return String(text ?? "")
    .replace(/работа пульта и индикации/gi, "работа блока управления и индикации")
    .replace(/Работает ли пульт управления без ошибок\?/g, "Работает ли блок управления без ошибок?")
...
function drawChamberMappingReport(doc: PDFKit.PDFDocument, source: ReportInput) {
  ...
  section("3. Область применения");
  renderTextBlock(doc,"Настоящий протокол применяется к стационарной холодильной камере для хранения лекарственных средств. Он охватывает проверку готовности камеры с применением дата-логгеров, исследование рабочего объёма, выявление неоднородности температуры и выбор мест постоянного мониторинга. Испытания с открыванием двери и отключением питания оцениваются в ходе аварийных проверок отдельно от проверки стабильной работы за 24 часа. В ходе суточной проверки стабильной работы камеры она эксплуатируется в штатном режиме, двери могут открываться и закрываться в рутинном порядке.");
  sub("4. Цели и задачи температурного картирования","Подтвердить поддержание заданного диапазона температуры в зоне размещения продукции; оценить стабильность и колебания; выявить холодную и горячую критические точки; определить участки с ограничениями хранения, в случае выявления; установить места средств измерения для постоянного мониторинга и необходимые корректирующие действия.");mark("4. Цели и задачи температурного картирования");
  ...
  sub("6.5. Определение точек размещения и оценка рисков","План 15+1: 8 углов рабочего объёма (C1–C8), 4 центра боковых граней (W1–W4), центры нижней и верхней граней (V1, V3) и центр объёма (V2). Такая расстановка охватывает длину, ширину и высоту хранения и позволяет сравнивать периферийные и центральные зоны. Риск-ориентированный подход допускается пунктом 16д Руководства ЕЭК.");
  renderTextBlock(doc,"Датчики фиксируют без прямого контакта с металлом, стенами и продукцией, не мешая рутинным операциям. Внешний регистратор располагают вне камеры в среде, которая воздействует на неё (например, в смежном помещении у двери).");
