...
export function drawTemperatureMapSummary(
  ...
  if (options.language === "en") {
    options.title = options.objectType === "truck"
      ? "Diagram 2. PQ/PV temperature map by average values"
      : "Diagram 2. Temperature map by average values";
  }
  ...
  doc.text(
    "Карта построена методом интерполяции по средним значениям внутренних регистраторов данных за период PQ/PV; числовые результаты и критерии приемки приведены в таблицах отчёта.",
    pageMargin,
    originY + designH * scale + 4,
    { width: doc.page.width - pageMargin * 2, align: "center" },
  );
  if (options.language === "en") {
    const captionY = originY + designH * scale + 1;
    doc.save();
    doc.rect(pageMargin - 2, captionY, doc.page.width - pageMargin * 2 + 4, 34).fill("#ffffff");
    doc.restore();
    doc.font("body").fontSize(7.5).fillColor("#64748b");
    doc.text(
      "The map was generated by interpolating the average readings of the internal data loggers for the PQ/PV period; numerical results and acceptance criteria are provided in the report tables.",
      pageMargin,
      captionY,
      { width: doc.page.width - pageMargin * 2, align: "center" },
    );
