...
describe("warehouse methodology logger table", () => {
  ...
  it("auto-fills pharmacy storage methodology sections from portal data", async () => {
    ...
    expect(text).toContain("Анализ расстановки датчиков и оценка рисков");
    expect(text).toContain("Внутренние датчики располагаются по выбранной схеме размещения помещения (зоны) хранения");
    expect(text).toContain("Такая многоточечная расстановка позволяет выявить температурные градиенты внутри помещения");
    expect(text).toContain("Внешний датчик установлен на улице и служит для мониторинга температуры окружающей среды");
    expect(text).toContain("не входит в расчёт основных критериев приемлемости этапа PQ/PV");
